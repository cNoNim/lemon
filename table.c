...
/* forward declaration */
static unsigned confighash(struct config *);
static int statecmp(struct config *, struct config *);
...
static unsigned confighash(struct config *a)
{
    unsigned h=0;
...
/* Compare two states */
static int statecmp(struct config *a, struct config *b)
{
    int rc;
...
/* Hash a state */
static unsigned statehash(struct config *a)
{
    unsigned h=0;
    while( a ){
        h = h*571 + a->rp->index*37 + a->dot;

...
** function.
*/
static char *file_makename(struct lemon *lemp, const char *suffix)
{
    char *name;
    char *cp;
    ...
    if( cp ){
        ...
        *cp = c;
        ...
        if( (pathbuf != 0) && (path!=0) ){
            ...
            while( *pathbuf ){
                cp = strchr(pathbuf,':');
                if( cp==0 ) cp = &pathbuf[strlen(pathbuf)];
                c = *cp;
                *cp = 0;
...
static void tplt_linedir(FILE *out, int lineno, char *filename)
{
...
static void tplt_print(FILE *out, struct lemon *lemp, char *str, int *lineno)
{
    if( str==0 ) return;
    while( *str ){
        putc(*str,out);
...
void ResortStates(struct lemon *lemp)
{
    ...
    qsort(&lemp->sorted[1], (size_t) (lemp->nstate-1), sizeof(lemp->sorted[0]),
            stateResortCompare);
    for(i=0; i<lemp->nstate; i++){

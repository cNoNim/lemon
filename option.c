...
/* forward declaration */
static int handleswitch(int, FILE *);
static int handleflags(int, FILE *);
static int argindex(int);
static void errline(int, size_t, FILE *);
...
OptPrint() {
  ...
  for (i = 0; op[i].label; i++) {
    ...
    case OPT_INT:
    case OPT_FINT:
      fprintf(errstream, "  -%s<integer>%*s  %s\n", op[i].label, (int)(max - strlen(op[i].label) - 9), "",
              op[i].message);
      break;
    case OPT_DBL:
    case OPT_FDBL:
      fprintf(errstream, "  -%s<real>%*s  %s\n", op[i].label, (int)(max - strlen(op[i].label) - 6), "", op[i].message);
      break;
    case OPT_STR:
    case OPT_FSTR:
      fprintf(errstream, "  -%s<string>%*s  %s\n", op[i].label, (int)(max - strlen(op[i].label) - 8), "",
              op[i].message);
      break;
    }
  }
